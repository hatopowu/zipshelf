...
- 一覧の上端にはステータスバー直下まで透明な先頭ジャンプ領域を置く。iOS のステータスバー自体は Web ページ外で直接タップ検出できないため、直下の上端余白をタップする。本棚では ZipShelf タイトル/冊数、サーバ取込では現在フォルダ名のタップでも、現在の一覧をアニメーションなしで即座に先頭へ戻す。
- **🔍絞り込み**(本棚・サーバ取込で共通): 小文字化＋空白区切りの AND 部分一致(`matchName`)。永続しない。iOS 対策として入力の `font-size` は **16px 必須**(未満だとフォーカス時に画面が拡大)、`.filt` で body の `user-select:none` を打ち消す(でないとキャレットが出ない)、ネイティブの検索クリアボタンは消して自前の ✕ に一本化。
  - 検索語は本棚・サーバ取込で共通の履歴として localStorage `zsh_searchHistory` に最近使った順・重複なしで最大30件保存する。検索欄のタップ/フォーカスで履歴を表示し、語のタップで適用、各行右端の × で個別削除、欄外のタップまたはフォーカス移動で閉じる。`.swrow` の `opacity` がスタッキングコンテキストを作るため、検索行自身を `.search-row` の `z-index` で一覧より前面に置く。
